...
function getMouseBoundedPosition(positionX, positionY, bounds, limitToBounds, paddingValue, wrapperComponent) {
...
function handleCalculateZoomPositions(contextInstance, mouseX, mouseY, newScale, bounds, limitToBounds) {
    ...
    var newPositions = getMouseBoundedPosition(calculatedPositionX, calculatedPositionY, bounds, limitToBounds, 0, null);
    return newPositions;
}
...
function handleNewPosition(contextInstance, newPositionX, newPositionY, paddingValue) {
    ...
    var _b = getMouseBoundedPosition(newPositionX, newPositionY, bounds, limitToBounds, paddingValue, wrapperComponent), x = _b.x, y = _b.y;
...
var getPaddingValue = function (contextInstance) {
    var setup = contextInstance.setup, transformState = contextInstance.transformState;
    var scale = transformState.scale;
    var minScale = setup.minScale, alignmentAnimation = setup.alignmentAnimation;
    var size = alignmentAnimation.size;
...
function handleVelocityPanning(contextInstance) {
    ...
    var alignAnimationTime = alignmentAnimation.velocityAlignmentTime;
    var moveAnimationTime = getVelocityMoveTime(contextInstance, total);
    var finalAnimationTime = Math.max(moveAnimationTime, alignAnimationTime);
    var paddingValue = getPaddingValue(contextInstance);
    var paddingX = (paddingValue * wrapperComponent.offsetWidth) / 100;
    var paddingY = (paddingValue * wrapperComponent.offsetHeight) / 100;
...
function handlePanning(contextInstance, clientX, clientY) {
    ...
    if (!startCoords)
        return;
    var _a = getPanningClientPosition(contextInstance, clientX, clientY), x = _a.x, y = _a.y;
    var paddingValue = getPaddingValue(contextInstance);
...
function handleAlignToBounds(contextInstance) {
    ...
    var disabled = alignmentAnimation.disabled, size = alignmentAnimation.size, animationTime = alignmentAnimation.animationTime, animationType = alignmentAnimation.animationType;
    var isDisabled = disabled || scale < minScale || !size;
    if (isDisabled)
        return;
...
function calculateZoomToNode(contextInstance, node) {
    var wrapperComponent = contextInstance.wrapperComponent;
    var limitToBounds = contextInstance.setup.limitToBounds;
    if (!wrapperComponent)
        return initialState;
    ...
    var bounds = calculateBounds(contextInstance, newScale);
    var _a = getMouseBoundedPosition(newPositionX, newPositionY, bounds, limitToBounds, 0, wrapperComponent), x = _a.x, y = _a.y;
...
var zoomIn = function (contextInstance) { return function (step, animationTime, animationName) {
    if (step === void 0) { step = 0.5; }
    if (animationTime === void 0) { animationTime = 300; }
    if (animationName === void 0) { animationName = "easeOut"; }
    handleZoomToViewCenter(contextInstance, 1, step, animationTime, animationName);
}; };
var zoomOut = function (contextInstance) { return function (step, animationTime, animationName) {
    if (step === void 0) { step = 0.5; }
    if (animationTime === void 0) { animationTime = 300; }
    if (animationName === void 0) { animationName = "easeOut"; }
    handleZoomToViewCenter(contextInstance, -1, step, animationTime, animationName);
...
var centerView = function (contextInstance) { return function () {
    var _a = contextInstance.props, initialPositionX = _a.initialPositionX, initialPositionY = _a.initialPositionY;
    var transformState = contextInstance.transformState, wrapperComponent = contextInstance.wrapperComponent, contentComponent = contextInstance.contentComponent;
    if (wrapperComponent && contentComponent) {
        var _b = getCenteredTransformStyles(initialPositionX, initialPositionY, transformState.scale, wrapperComponent, contentComponent), transform = _b.transform, positionsState = _b.positionsState;
        contentComponent.style.transform = transform;
        contextInstance.transformState = __assign(__assign({}, contextInstance.transformState), positionsState);
    }
}; };
var zoomToElement = function (contextInstance) { return function (node, animationTime, animationName) {
    ...
    if (wrapperComponent && target && wrapperComponent.contains(target)) {
        var targetState = calculateZoomToNode(contextInstance, target);
...
var getCenteredTransformStyles = function (initialPositionX, initialPositionY, scale, wrapperComponent, contentComponent) {
    var _a = getCenterPosition(wrapperComponent, contentComponent), centerPositionX = _a.centerPositionX, centerPositionY = _a.centerPositionY;
    var x = initialPositionX !== null && initialPositionX !== void 0 ? initialPositionX : centerPositionX;
    var y = initialPositionY !== null && initialPositionY !== void 0 ? initialPositionY : centerPositionY;
    return {
        transform: "translate3d(" + x + "px, " + y + "px, 0) scale(" + scale + ")",
        positionsState: { positionX: x, positionY: y },
    };
};
var getCenterPosition = function (wrapperComponent, contentComponent) {
    var wrapperRect = wrapperComponent.getBoundingClientRect();
    var contentRect = contentComponent.getBoundingClientRect();
    var centerPositionX = (wrapperRect.width - contentRect.width) / 2;
    var centerPositionY = (wrapperRect.height - contentRect.height) / 2;
    return { centerPositionX: centerPositionX, centerPositionY: centerPositionY };
};
...
var Context = React__default['default'].createContext(contextInitialState);
var TransformContext = /** @class */ (function (_super) {
    ...
    TransformContext.prototype.render = function () {
        ...
        return (jsxRuntime.jsx(Context.Provider, __assign({ value: __assign(__assign({}, this.transformState), { setComponents: this.setComponents, contextInstance: this }) }, { children: content }), void 0));
    };
    return TransformContext;
}(React.Component));

var TransformWrapper = React__default['default'].forwardRef(function (props, ref) {
    var _a = React.useState(null), innerRef = _a[0], setRef = _a[1];
    React.useImperativeHandle(ref, function () { return innerRef; }, [innerRef]);
    return jsxRuntime.jsx(TransformContext, __assign({}, props, { setRef: setRef }), void 0);
});
...
var TransformComponent = function (_a) {
    ...
    return (jsxRuntime.jsx("div", __assign({ ref: wrapperRef, className: "react-transform-wrapper " + styles.wrapper + " " + wrapperClass, style: wrapperStyle }, { children: jsxRuntime.jsx("div", __assign({ ref: contentRef, className: "react-transform-component " + styles.content + " " + contentClass, style: contentStyle }, { children: children }), void 0) }), void 0));
};

var useTransformContext = function () {
    var libraryContext = React.useContext(Context);
...
exports.useTransformContext = useTransformContext;
